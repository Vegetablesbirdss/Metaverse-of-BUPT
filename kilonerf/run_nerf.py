...
def render(intrinsics, chunk=1024*32, rays=None, c2w=None, ndc=False,
...
def render_path(render_poses, intrinsics, chunk, render_kwargs, gt_imgs=None, savedir=None, render_factor=0):
    ...
    if use_fast_sampling:
        print("AAAAAAAAAAAAAAAAAAAAAAA", c2ws[0], intrinsics, render_kwargs)
    ...
    mse_list, psnr_list, ssim_list, elapsed_time_list = [], [], [], []
    ...
    for i, c2w in enumerate(tqdm(c2ws)):
        ...
        if calculate_quality_metrics:
            ...
            #print(rgb.shape, gt_img_pytorch)
            #lpips = LPIPS.calculate(rgb, gt_img_pytorch)
            log_str += 'MSE: {:.6f}, PSNR: {:.3f}, SSIM: {:.3f}, '.format(mse.item(), psnr.item(), ssim)
    ...
    average_mse = average_psnr = average_ssim =  0
    if calculate_quality_metrics:
        average_mse = sum(mse_list) / len(mse_list)
        average_psnr = sum(psnr_list) / len(psnr_list)
        average_ssim = sum(ssim_list) / len(ssim_list)
    average_elapsed_time = sum(elapsed_time_list) / len(elapsed_time_list)
    Logger.write('average over image batch. MSE: {:.6f}, PSNR: {:.3f}, SSIM: {:.3f}, elapsed time: {:7.2f} ms'.format(
        average_mse, average_psnr, average_ssim, average_elapsed_time * 1000))
    if savedir is not None:
        numerical_results = {'mse': average_mse, 'psnr': average_psnr, 'ssim': average_ssim, 'elapsed_time': average_elapsed_time}
...
def train(cfg, log_path, render_cfg_path):
    ...
    print("OMOMOMOMOMOMOMOMOMOMOMOMOMOMOMOMOMOMOMO")
    ...
    if cfg['dataset_type'] != 'nsvf':
        ...
        del H, W, focal, hwf
    print(near, far, background_color)
    ...
    if cfg['render_only']:
        ...
        with torch.no_grad():
            ...
            print('test poses shapeHHHHHHHHHHHHHHHHHH', render_poses.shape)
