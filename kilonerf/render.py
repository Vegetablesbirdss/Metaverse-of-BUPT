...
def w2c(x, y, z, sita, beta, fi):
    """
    sita is the angle between xs
    beta is the angle between ys
    fi is the angle between zs
    """

    m_of_s = np.matrix([[1, 0, 0], [0, math.cos(sita), math.sin(sita)],
                        [0, -math.sin(sita), math.cos(sita)]])
    m_of_b = np.matrix([[math.cos(beta), 0, -math.sin(beta)],
                        [0, 1, 0], [math.sin(beta), 0, math.cos(beta)]])
    m_of_f = np.matrix([[math.cos(fi), math.sin(fi), 0],
                        [-math.sin(fi), math.cos(fi), 0], [0, 0, 1]])
    m_of_w = np.matrix([x, y, z, 1])
    # marix of rota is the multi of matixs of angles
    matrixofR = np.dot(np.dot(m_of_s, m_of_b), m_of_f)

    t = np.matrix([x, y, z])

    tem = np.column_stack((matrixofR, t.T))

    R = np.row_stack((tem, np.matrix([0, 0, 0, 1])))

    m_of_c = np.dot(R, m_of_w.T)
...
def anti_w2c(R):
    ...
    if(sinb == 0):
        ...
        beta = math.asin(sinb)
        cosb = math.cos(beta)
    fi = math.asin(R[0, 1]/cosb)
    sita = math.acos(R[2, 2]/cosb)

    return np.matrix([x, y, z, sita, beta, fi])
...
#------------------------------------------------------------------------------------------------------------------------------
#The former part is manily about the model loading and initializing process, and the following part is using the pre-trained model to generate the 2d-photos

# Move testing data to GPU  

def render(x, y, z, th1, th2, th3=0):
    render_poses = torch.Tensor(w2c(x, y, z, th1, th2, th3)).to(device)
# render_poses = torch.Tensor([[-1.0000,  0.0000,  0.0000,  0.0000],[0.0000, -0.7341,  0.6790,  2.7373],[0.0000,  0.6790,  0.7341,  2.9593],[0.0000,  0.0000,  0.0000,  1.0000]]).to(device)
    print(render_poses)
    render_poses = torch.unsqueeze(render_poses, 0)  
    ...
    with torch.no_grad():
        ...
        testsavedir = log_path + '/render_FSR'
        if render_cfg_path is not None:
        ...
        rgbs, _, filename = render_path(render_poses, intrinsics, cfg['chunk_size'], render_kwargs_test, gt_imgs=images,
...
def post():
    ...
    print(data)
    imgurl = render(data[0],data[1],data[2],data[3]*3.141592653/180,data[4]*3.141592653/180)
    #fileurl = "../static/pics_temp/" + str(cnt) + ".jpg"
    print(imgurl)
    args = {'imgurl': imgurl, 'time': time()}
